...
struct FrameSparseMatrices {
    Eigen::SparseMatrix<double, Eigen::RowMajor, int> M_L_C_L; // Modulator Left convolved with Carrier Left
    Eigen::SparseMatrix<double, Eigen::RowMajor, int> M_L_C_R; // Modulator Left convolved with Carrier Right
    Eigen::SparseMatrix<double, Eigen::RowMajor, int> M_R_C_L; // Modulator Right convolved with Carrier Left
    Eigen::SparseMatrix<double, Eigen::RowMajor, int> M_R_C_R; // Modulator Right convolved with Carrier Right

    FrameSparseMatrices(int rows, int cols) 
        : M_L_C_L(rows, cols), M_L_C_R(rows, cols),
          M_R_C_L(rows, cols), M_R_C_R(rows, cols) {}
};
...
struct FrameDeltaSparseMatrices {
    Eigen::SparseMatrix<double, Eigen::RowMajor, int> Delta_M_L_C_L; // Delta Modulator Left convolved with Carrier Left
    Eigen::SparseMatrix<double, Eigen::RowMajor, int> Delta_M_L_C_R; // Delta Modulator Left convolved with Carrier Right
    Eigen::SparseMatrix<double, Eigen::RowMajor, int> Delta_M_R_C_L; // Delta Modulator Right convolved with Carrier Left
    Eigen::SparseMatrix<double, Eigen::RowMajor, int> Delta_M_R_C_R; // Delta Modulator Right convolved with Carrier Right

    FrameDeltaSparseMatrices(int rows, int cols) 
        : Delta_M_L_C_L(rows, cols), Delta_M_L_C_R(rows, cols),
          Delta_M_R_C_L(rows, cols), Delta_M_R_C_R(rows, cols) {}
};
...
void serializeDeltaDataStream(const std::vector<FrameDeltaSparseMatrices>& deltaDataStream, const std::string& filename) {
    ...
    for (const auto& frame : deltaDataStream) {
        serializeSparseMatrix(frame.Delta_M_L_C_L, ofs);
        serializeSparseMatrix(frame.Delta_M_L_C_R, ofs);
        serializeSparseMatrix(frame.Delta_M_R_C_L, ofs);
        serializeSparseMatrix(frame.Delta_M_R_C_R, ofs);
    }
...
std::vector<FrameDeltaSparseMatrices> deserializeDeltaDataStream(const std::string& filename) {
    ...
    for (int i = 0; i < numFrames; ++i) {
        ...
        deltaFrame.Delta_M_L_C_L = deserializeSparseMatrix(ifs);
        deltaFrame.Delta_M_L_C_R = deserializeSparseMatrix(ifs);
        deltaFrame.Delta_M_R_C_L = deserializeSparseMatrix(ifs);
        deltaFrame.Delta_M_R_C_R = deserializeSparseMatrix(ifs);
...
std::vector<FrameSparseMatrices> reconstructDataStream(const std::vector<FrameDeltaSparseMatrices>& deltaDataStream, int Nfft) {
    ...
    previousFrame.M_L_C_L.setZero();
    previousFrame.M_L_C_R.setZero();
    previousFrame.M_R_C_L.setZero();
    previousFrame.M_R_C_R.setZero();
    
    for (const auto& deltaFrame : deltaDataStream) {
        FrameSparseMatrices currentFrame(Nfft, Nfft);
        
        // Add deltas to previous frame
        currentFrame.M_L_C_L = previousFrame.M_L_C_L + deltaFrame.Delta_M_L_C_L;
        currentFrame.M_L_C_R = previousFrame.M_L_C_R + deltaFrame.Delta_M_L_C_R;
        currentFrame.M_R_C_L = previousFrame.M_R_C_L + deltaFrame.Delta_M_R_C_L;
        currentFrame.M_R_C_R = previousFrame.M_R_C_R + deltaFrame.Delta_M_R_C_R;
...
int main() {
    // File paths for two stereo WAV files (modulator and carrier)
    std::string modulatorFile = "../data/modulatorStereo.wav"; // Replace with your modulator file path
    std::string carrierFile = "../data/carrierStereo.wav";     // Replace with your carrier file path
    ...
    // Vectors to hold left and right channels for modulator and carrier
    std::vector<double> mod_left, mod_right;
    std::vector<double> car_left, car_right;
    int fs_mod, fs_car;
    
    // Read modulator WAV file
    if (!readStereoWavFile(modulatorFile, mod_left, mod_right, fs_mod)) {
        return -1;
    }
    
    // Read carrier WAV file
    if (!readStereoWavFile(carrierFile, car_left, car_right, fs_car)) {
        return -1;
    }
    
    // Ensure both files have the same sampling rate and length
    if (fs_mod != fs_car) {
        std::cerr << "Sampling rates of modulator and carrier do not match." << std::endl;
        return -1;
    }
    
    size_t min_size = std::min({mod_left.size(), mod_right.size(), car_left.size(), car_right.size()});
    mod_left.resize(min_size, 0.0);
    mod_right.resize(min_size, 0.0);
    car_left.resize(min_size, 0.0);
    car_right.resize(min_size, 0.0);
    
    // Generate white noise carrier signals using NEON
    generateWhiteNoiseNEON(car_left, 0.7);
    generateWhiteNoiseNEON(car_right, 0.7);
    ...
    // Initialize accumulators for modulator and carrier
    std::vector<double> mod_left_accum(min_size, 0.0);
    std::vector<double> mod_right_accum(min_size, 0.0);
    std::vector<double> car_left_accum(min_size, 0.0);
    std::vector<double> car_right_accum(min_size, 0.0);
    ...
    // Processing loop with delta compression
    for (int i = 0; i < number_of_frames; ++i) {
        int begin_sample = i * hopSize;
        int end_sample = begin_sample + frame_duration;
        
        // Extract frames for modulator and carrier (left and right channels)
        std::vector<double> mod_frame_left(mod_left.begin() + begin_sample, mod_left.begin() + end_sample);
        std::vector<double> mod_frame_right(mod_right.begin() + begin_sample, mod_right.begin() + end_sample);
        std::vector<double> car_frame_left(car_left.begin() + begin_sample, car_left.begin() + end_sample);
        std::vector<double> car_frame_right(car_right.begin() + begin_sample, car_right.begin() + end_sample);
        ...
        for (; j + 1 < frame_duration; j += 2) {
            ...
            // Load two samples for modulator left
            float64x2_t samp_mod_left = vld1q_f64(&mod_frame_left[j]);
            // Apply window
            float64x2_t windowed_mod_left = vmulq_f64(samp_mod_left, win);
            // Store back
            vst1q_f64(&mod_frame_left[j], windowed_mod_left);
            
            // Load two samples for modulator right
            float64x2_t samp_mod_right = vld1q_f64(&mod_frame_right[j]);
            // Apply window
            float64x2_t windowed_mod_right = vmulq_f64(samp_mod_right, win);
            // Store back
            vst1q_f64(&mod_frame_right[j], windowed_mod_right);
            
            // Load two samples for carrier left
            float64x2_t samp_car_left = vld1q_f64(&car_frame_left[j]);
            // Apply window
            float64x2_t windowed_car_left = vmulq_f64(samp_car_left, win);
            // Store back
            vst1q_f64(&car_frame_left[j], windowed_car_left);
            
            // Load two samples for carrier right
            float64x2_t samp_car_right = vld1q_f64(&car_frame_right[j]);
            // Apply window
            float64x2_t windowed_car_right = vmulq_f64(samp_car_right, win);
            // Store back
            vst1q_f64(&car_frame_right[j], windowed_car_right);
        }
        
        // Handle remaining samples if frame_duration is odd
        for (; j < frame_duration; ++j) {
            mod_frame_left[j] *= hamming_window[j];
            mod_frame_right[j] *= hamming_window[j];
            car_frame_left[j] *= hamming_window[j];
            car_frame_right[j] *= hamming_window[j];
        }
        
        // Pad frames to Nfft with zeros if necessary
        if (mod_frame_left.size() < Nfft) {
            mod_frame_left.resize(Nfft, 0.0);
            mod_frame_right.resize(Nfft, 0.0);
            car_frame_left.resize(Nfft, 0.0);
            car_frame_right.resize(Nfft, 0.0);
        }
        
        // Compute FFT for modulator left
        std::vector<std::complex<double>> fft_mod_left = computeFFT(mod_frame_left, fft_plan, in_fft, out_fft);
        
        // Compute FFT for modulator right
        std::vector<std::complex<double>> fft_mod_right = computeFFT(mod_frame_right, fft_plan, in_fft, out_fft);
        
        // Compute FFT for carrier left
        std::vector<std::complex<double>> fft_car_left = computeFFT(car_frame_left, fft_plan, in_fft, out_fft);
        
        // Compute FFT for carrier right
        std::vector<std::complex<double>> fft_car_right = computeFFT(car_frame_right, fft_plan, in_fft, out_fft);
        
        // Inverse FFT and accumulate to modulator accumulators
        std::vector<double> ifft_mod_left = computeIFFT(fft_mod_left, ifft_plan, in_ifft, out_ifft);
        std::vector<double> ifft_mod_right = computeIFFT(fft_mod_right, ifft_plan, in_ifft, out_ifft);
        std::vector<double> ifft_car_left = computeIFFT(fft_car_left, ifft_plan, in_ifft, out_ifft);
        std::vector<double> ifft_car_right = computeIFFT(fft_car_right, ifft_plan, in_ifft, out_ifft);
        
        // Accumulate using NEON
        accumulateToOutputNEON(mod_left_accum, ifft_mod_left, begin_sample, frame_duration);
        accumulateToOutputNEON(mod_right_accum, ifft_mod_right, begin_sample, frame_duration);
        accumulateToOutputNEON(car_left_accum, ifft_car_left, begin_sample, frame_duration);
        accumulateToOutputNEON(car_right_accum, ifft_car_right, begin_sample, frame_duration);
        
        // Cepstral Analysis for Modulator Left
        std::vector<double> magnitude_spectrumM_L(frame_duration, 0.0);
        for (int n = 0; n < frame_duration; ++n) {
            magnitude_spectrumM_L[n] = std::log(std::abs(fft_mod_left[n]));
        }
        
        // Compute IFFT of log magnitude for Modulator Left
        for (int n = 0; n < frame_duration; ++n) {
            in_ifft[n][0] = magnitude_spectrumM_L[n];
            in_ifft[n][1] = 0.0;
        }
        fftw_execute(ifft_plan);
        Eigen::VectorXd cepM_L = Eigen::VectorXd::Zero(number_of_features);
        for (int n = 0; n < number_of_features; ++n) {
            cepM_L(n) = out_ifft[n];
        }
        
        // Cepstral Analysis for Modulator Right
        std::vector<double> magnitude_spectrumM_R(frame_duration, 0.0);
        for (int n = 0; n < frame_duration; ++n) {
            magnitude_spectrumM_R[n] = std::log(std::abs(fft_mod_right[n]));
        }
        
        // Compute IFFT of log magnitude for Modulator Right
        for (int n = 0; n < frame_duration; ++n) {
            in_ifft[n][0] = magnitude_spectrumM_R[n];
            in_ifft[n][1] = 0.0;
        }
        fftw_execute(ifft_plan);
        Eigen::VectorXd cepM_R = Eigen::VectorXd::Zero(number_of_features);
        for (int n = 0; n < number_of_features; ++n) {
            cepM_R(n) = out_ifft[n];
        }
        
        // Cepstral Analysis for Carrier Left
        std::vector<double> magnitude_spectrumC_L(frame_duration, 0.0);
        for (int n = 0; n < frame_duration; ++n) {
            magnitude_spectrumC_L[n] = std::log(std::abs(fft_car_left[n]));
        }
        
        // Compute IFFT of log magnitude for Carrier Left
        for (int n = 0; n < frame_duration; ++n) {
            in_ifft[n][0] = magnitude_spectrumC_L[n];
            in_ifft[n][1] = 0.0;
        }
        fftw_execute(ifft_plan);
        Eigen::VectorXd cepC_L = Eigen::VectorXd::Zero(number_of_features);
        for (int n = 0; n < number_of_features; ++n) {
            cepC_L(n) = out_ifft[n];
        }
        
        // Cepstral Analysis for Carrier Right
        std::vector<double> magnitude_spectrumC_R(frame_duration, 0.0);
        for (int n = 0; n < frame_duration; ++n) {
            magnitude_spectrumC_R[n] = std::log(std::abs(fft_car_right[n]));
        }
        
        // Compute IFFT of log magnitude for Carrier Right
        for (int n = 0; n < frame_duration; ++n) {
            in_ifft[n][0] = magnitude_spectrumC_R[n];
            in_ifft[n][1] = 0.0;
        }
        fftw_execute(ifft_plan);
        Eigen::VectorXd cepC_R = Eigen::VectorXd::Zero(number_of_features);
        for (int n = 0; n < number_of_features; ++n) {
            cepC_R(n) = out_ifft[n];
        }
        
        // Reconstruct the spectrum from cepstrum for Modulator Left
        Eigen::VectorXd cepM_padded_L = Eigen::VectorXd::Zero(Nfft);
        cepM_padded_L.head(number_of_features) = cepM_L;
        std::vector<double> cepM_padded_vec_L(Nfft, 0.0);
        for (int n = 0; n < number_of_features; ++n) {
            cepM_padded_vec_L[n] = cepM_padded_L(n);
        }
        std::vector<std::complex<double>> reconstructed_spectrumM_L = computeFFT(cepM_padded_vec_L, fft_plan, in_fft, out_fft);
        // Exponentiate to get magnitude spectrum
        for (auto& val : reconstructed_spectrumM_L) {
            val = std::exp(val.real()) + std::complex<double>(0.0, 0.0);
        }
        
        // Reconstruct the spectrum from cepstrum for Modulator Right
        Eigen::VectorXd cepM_padded_R = Eigen::VectorXd::Zero(Nfft);
        cepM_padded_R.head(number_of_features) = cepM_R;
        std::vector<double> cepM_padded_vec_R(Nfft, 0.0);
        for (int n = 0; n < number_of_features; ++n) {
            cepM_padded_vec_R[n] = cepM_padded_R(n);
        }
        std::vector<std::complex<double>> reconstructed_spectrumM_R = computeFFT(cepM_padded_vec_R, fft_plan, in_fft, out_fft);
        // Exponentiate to get magnitude spectrum
        for (auto& val : reconstructed_spectrumM_R) {
            val = std::exp(val.real()) + std::complex<double>(0.0, 0.0);
        }
        
        // Reconstruct the spectrum from cepstrum for Carrier Left
        Eigen::VectorXd cepC_padded_L_vec = Eigen::VectorXd::Zero(Nfft);
        cepC_padded_L_vec.head(number_of_features) = cepC_L;
        std::vector<double> cepC_padded_vec_L(Nfft, 0.0);
        for (int n = 0; n < number_of_features; ++n) {
            cepC_padded_vec_L[n] = cepC_padded_L_vec(n);
        }
        std::vector<std::complex<double>> reconstructed_spectrumC_L = computeFFT(cepC_padded_vec_L, fft_plan, in_fft, out_fft);
        // Exponentiate to get magnitude spectrum
        for (auto& val : reconstructed_spectrumC_L) {
            val = std::exp(val.real()) + std::complex<double>(0.0, 0.0);
        }
        
        // Reconstruct the spectrum from cepstrum for Carrier Right
        Eigen::VectorXd cepC_padded_R_vec = Eigen::VectorXd::Zero(Nfft);
        cepC_padded_R_vec.head(number_of_features) = cepC_R;
        std::vector<double> cepC_padded_vec_R(Nfft, 0.0);
        for (int n = 0; n < number_of_features; ++n) {
            cepC_padded_vec_R[n] = cepC_padded_R_vec(n);
        }
        std::vector<std::complex<double>> reconstructed_spectrumC_R = computeFFT(cepC_padded_vec_R, fft_plan, in_fft, out_fft);
        // Exponentiate to get magnitude spectrum
        for (auto& val : reconstructed_spectrumC_R) {
            val = std::exp(val.real()) + std::complex<double>(0.0, 0.0);
        }
        
        // Interpolate envelopes to Nfft
        std::vector<double> smoothM_L = linearInterpolate(std::vector<double>(cepM_padded_vec_L.begin(), cepM_padded_vec_L.begin() + number_of_features), Nfft);
        std::vector<double> smoothM_R = linearInterpolate(std::vector<double>(cepM_padded_vec_R.begin(), cepM_padded_vec_R.begin() + number_of_features), Nfft);
        std::vector<double> smoothC_L = linearInterpolate(std::vector<double>(cepC_padded_vec_L.begin(), cepC_padded_vec_L.begin() + number_of_features), Nfft);
        std::vector<double> smoothC_R = linearInterpolate(std::vector<double>(cepC_padded_vec_R.begin(), cepC_padded_vec_R.begin() + number_of_features), Nfft);
        
        // Flatten carrier spectra using NEON
        std::vector<std::complex<double>> flat_frameC_L = flattenCarrierSpectrumNEON(reconstructed_spectrumC_L, smoothC_L);
        std::vector<std::complex<double>> flat_frameC_R = flattenCarrierSpectrumNEON(reconstructed_spectrumC_R, smoothC_R);
        
        // Multiply with modulator's envelopes using NEON
        std::vector<std::complex<double>> XS_L = multiplyWithModEnvelopeNEON(flat_frameC_L, smoothM_L);
        std::vector<std::complex<double>> XS_R = multiplyWithModEnvelopeNEON(flat_frameC_R, smoothM_R);
        
        // Perform IFFT to get time-domain signals
        std::vector<double> ifft_resultXS_L = computeIFFT(XS_L, ifft_plan, in_ifft, out_ifft);
        std::vector<double> ifft_resultXS_R = computeIFFT(XS_R, ifft_plan, in_ifft, out_ifft);
        
        // Accumulate to xs_output using NEON
        accumulateToOutputNEON(xs_output, ifft_resultXS_L, begin_sample, frame_duration);
        accumulateToOutputNEON(xs_output, ifft_resultXS_R, begin_sample, frame_duration);
        ...
        // Modulator Left * Carrier Left
        for (int n = 0; n < Nfft; ++n) {
            double value = std::abs(reconstructed_spectrumM_L[n] * reconstructed_spectrumC_L[n]);
            if (value > threshold) {
                currentFrame.M_L_C_L.insert(n, n) = value;
            }
        }
        
        // Modulator Left * Carrier Right
        for (int n = 0; n < Nfft; ++n) {
            double value = std::abs(reconstructed_spectrumM_L[n] * reconstructed_spectrumC_R[n]);
            if (value > threshold) {
                currentFrame.M_L_C_R.insert(n, n) = value;
            }
        }
        
        // Modulator Right * Carrier Left
        for (int n = 0; n < Nfft; ++n) {
            double value = std::abs(reconstructed_spectrumM_R[n] * reconstructed_spectrumC_L[n]);
            if (value > threshold) {
                currentFrame.M_R_C_L.insert(n, n) = value;
            }
        }
        
        // Modulator Right * Carrier Right
        for (int n = 0; n < Nfft; ++n) {
            double value = std::abs(reconstructed_spectrumM_R[n] * reconstructed_spectrumC_R[n]);
            if (value > threshold) {
                currentFrame.M_R_C_R.insert(n, n) = value;
            }
        }
        
        // Compute deltas
        FrameDeltaSparseMatrices deltaFrame(Nfft, Nfft);
        deltaFrame.Delta_M_L_C_L = computeDelta(currentFrame.M_L_C_L, previousFrame.M_L_C_L, 1e-6);
        deltaFrame.Delta_M_L_C_R = computeDelta(currentFrame.M_L_C_R, previousFrame.M_L_C_R, 1e-6);
        deltaFrame.Delta_M_R_C_L = computeDelta(currentFrame.M_R_C_L, previousFrame.M_R_C_L, 1e-6);
        deltaFrame.Delta_M_R_C_R = computeDelta(currentFrame.M_R_C_R, previousFrame.M_R_C_R, 1e-6);
        
        // Store the delta frame
        deltaDataStream.push_back(deltaFrame);
        
        // Store the current frame in dataStream (optional, if you need to keep the full data)
    ...
    /*
    if (!writeWavFile(outputFile, xs_output, fs_mod)) {
        return -1;
    }
